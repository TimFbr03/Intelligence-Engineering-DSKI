...
def encode_labels(df):
    ...
    # Stelle sicher, dass NaN-Werte ersetzt werden
    for col in ["type", "queue", "priority"]:
        # Ersetze NaN oder None mit 'unknown'
        df[col] = df[col].fillna('unknown')  # Setze NaN-Werte auf 'unknown'
        
        # Überprüfe nach der Ersetzung die einzigartigen Werte
        print(f"Einzigartige Werte in '{col}' nach NaN-Ersetzung:")
        print(df[col].unique())

        # Sortiere die einzigartigen Werte und erstelle das Label-Encoder-Dictionary
        classes = sorted(df[col].unique())
        label_encoders[col] = {c: i for i, c in enumerate(classes)}
        
        # Ersetze die Kategorien durch numerische Labels
...
def tokenize_dataset(train_df, val_df, test_df):
    ...
    # Umwandeln der DataFrames in Datasets
    ...
    def tokenize_function(examples):
        ...
        # Bereite die Labels vor (jetzt als Listen zurückgeben)
        tokenized["labels_type"] = examples["type"]
        tokenized["labels_queue"] = examples["queue"]
        tokenized["labels_priority"] = examples["priority"]

        return tokenized

    # Wende die Tokenisierung auf die Datensätze an
    # 'batched=True' sorgt dafür, dass die Funktion auf Batches angewendet wird
